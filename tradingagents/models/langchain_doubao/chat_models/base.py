...
from tradingagents.models.langchain_doubao.chat_models._client_utils import (
    _get_default_async_httpx_client,
    _get_default_httpx_client,
)
from tradingagents.models.langchain_doubao.chat_models._compat import (
    _convert_from_v03_ai_message,
    _convert_to_v03_ai_message,
)
from volcenginesdkarkruntime import Ark, AsyncArk
from volcenginesdkarkruntime._exceptions import ArkBadRequestError
from volcenginesdkarkruntime._models import BaseModel as ArkBaseModel
...
def _convert_dict_to_message(_dict: Mapping[str, Any]) -> BaseMessage:
    ...
    if role == "user":
        ...
        if role == "developer":
            additional_kwargs = {"__ark_role__": role}
        else:
            additional_kwargs = {}
...
def _convert_delta_to_message_chunk(
    ...
    if role == "user" or default_class == HumanMessageChunk:
        ...
        if role == "developer":
            additional_kwargs = {"__ark_role__": "developer"}
        else:
            additional_kwargs = {}
...
def _handle_ark_bad_request(e: ArkBadRequestError) -> None:
    if (
...
class BaseChatDouBao(BaseChatModel):
    ...
    in :meth:`~tradingagents.models.langchain_doubao.chat_models.base.ChatDouBao.with_structured_output`.
    ...
    def validate_environment(self) -> Self:
        ...
        if not self.client:
            ...
            sync_specific = {
                "http_client": self.http_client
                or _get_default_httpx_client(self.ark_api_base, self.request_timeout)
            }
            # type: ignore[arg-type]
            self.root_client = Ark(api_key=self.ark_api_key.get_secret_value())
        ...
        if not self.async_client:
            ...
            async_specific = {
                ...
                or _get_default_async_httpx_client(
                    self.ark_api_base, self.request_timeout
                )
            }
            self.root_async_client = AsyncArk(
                api_key=self.ark_api_key.get_secret_value()
            )
    ...
    def _create_chat_result(
        self,
        response: Union[dict, ArkBaseModel],
        ...
        if isinstance(response, ArkBaseModel) and getattr(
    ...
    @deprecated(
        since="0.2.1",
        alternative="tradingagents.models.langchain_doubao.chat_models.base.ChatDouBao.bind_tools",
        removal="1.0.0",
    )
    def bind_functions(
        self,
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        if method == "function_calling":
            ...
            response_format = _convert_to_ark_response_format(schema, strict=strict)
            bind_kwargs = {
                **dict(
...
class ChatDouBao(BaseChatDouBao):  # type: ignore[override]
    ...
    def get_lc_namespace(cls) -> list[str]:
        ...
        return ["langchain", "chat_models", "ark"]
...
def _convert_to_ark_response_format(
                ...
                if tool["type"] == "image_generation":
                    ...
                    if "partial_images" in tool:
                        raise NotImplementedError(
                            ...
                            "via the LangChain ChatDouBao client. Please "
    ...
    if schema := payload.pop("response_format", None):
        ...
        if not payload.get("stream") and _is_pydantic_class(schema):
            ...
            if schema_dict == {"type": "json_object"}:  # JSON mode
                ...
                (
                    response_format := _convert_to_ark_response_format(
